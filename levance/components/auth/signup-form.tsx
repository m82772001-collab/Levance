  ...
  return (
    <form action={formAction} className="space-y-5">
      <div className="text-center">
        <p className="eyebrow mb-3">Join LÉVANCE</p>
        <h1 className="font-display text-3xl tracking-tight">Create account</h1>
        <p className="mt-2 text-sm text-neutral-500">
          You begin with the free COMMON membership.
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        {state.fieldErrors?.fullName && (
          <p className="text-xs text-danger">{state.fieldErrors.fullName[0]}</p>
        )}
      </div>
      ...
      <label className="flex items-start gap-3 text-sm text-neutral-600">
        <input
          ...
          value="true"
          required
          className="mt-1 h-4 w-4 rounded border-neutral-300 text-obsidian focus:ring-champagne"
        />
        <span>
          I accept the{" "}
          <Link href="/terms" className="underline underline-offset-2 hover:text-obsidian">
            terms of service
          </Link>{" "}
          and{" "}
          <Link href="/privacy" className="underline underline-offset-2 hover:text-obsidian">
            privacy policy
          </Link>
          .
        </span>
      ...
      {state.fieldErrors?.acceptedTerms && (
        <p className="text-xs text-danger">{state.fieldErrors.acceptedTerms[0]}</p>
      )}
